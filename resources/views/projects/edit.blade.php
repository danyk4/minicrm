                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <div>
                            ...
                            <textarea id="description" class="block mt-1 w-full border-gray-300" type="text" name="description" required autofocus autocomplete="description">{{  old('description', $project->description) }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <select class="block mt-1 w-full" name="status">
                                @foreach(\App\Enums\ProjectStatus::cases() as $status)
                                    <option value="{{ $status->value }}" @selected(old('status', $project->status->value) == $status->value)>{{ $status }}</option>
                                @endforeach
                            </select>
